...
### E1 — DeepSeek dev key EXPIRED (401) — blocked live classify-WRITE verification
The localStorage `llm:key` `sk-...dd3a` returns HTTP 401 from `api.deepseek.com/v1/chat/completions`: `{"error":{"message":"Authentication Fails, Your api key: ****dd3a is invalid","type":"authentication_error"}}`. Observed on BOTH the fresh capture (entry → `处理失败`, D7 path) AND 6 summary `recomputeAggregate` calls (→ D4). Older entries show "AI 已分类" because they were classified in a prior session when the key was valid. The STT key `stt:key` (`sk-ws-...`) was not exercised (no voice capture attempted — getUserMedia env-limited). **Impact on this gate:** D1/D3/D4 (classify-WRITE: version increment, aggregate entryIds, detailLevel clamp) could NOT be live-verified end-to-end — only their READ/render path was verified on already-classified entries. D7 (failure-marking) and D9-adjacent (D4 above) WERE verified. **Recommend lead refresh the dev DeepSeek key (and re-run a minimal capture→classify→detail sweep) before ship.**
