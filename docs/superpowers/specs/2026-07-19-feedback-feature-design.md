...
- **安全约束（开发者必须遵守）**：fine-grained PAT，仅授 `issues:write` + `gists:write`，限定 AiJi 仓库，90 天过期，不勾 admin/contents/pulls。
...
2. 收集本次提交所有图片 → 一次 `POST /gists`（public=false，文件名 `img-{i}-{rand}.jpg`，base64 content）→ 取每文件 `raw_url`。
3. 拼 markdown body：`> 来自 AiJi App 使用反馈 · {ts}` + 每条 `## 建议 N` + 文字 + `![](raw_url)`。
4. `POST /repos/{owner}/{repo}/issues`，title = 首条文字前 40 字，labels=`['feedback']`。
5. 返回 `html_url`。

**图片外链选 gist**：`gist.githubusercontent.com` raw URL 经 camo 在 Issue markdown 内渲染（已验证路径）。若实现期验证发现不渲染，回退到 repo 孤儿分支 `feedback-assets` + `raw.githubusercontent.com`（PAT 加 `contents:write`，scope 更宽故非首选）。**1 次提交 = 1 Issue**，body 内 N 段（不刷屏，Claude Code 按段处理）。
...
- Vitest 单测：`buildBody`（分段+图链）、`blobToBase64`、gist payload 构造（mock fetch）。
- E2E（390×844）：`/feedback` → 2 条建议各带 1 图 → mock `fetch`（gist + issue）→ 断言成功态显 URL + issue body 含两段+图链。
